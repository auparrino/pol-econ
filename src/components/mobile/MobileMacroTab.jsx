  ...
  return (
    <div className="absolute inset-0 flex flex-col">
      <div className="shrink-0 flex items-center" style={{ height: 48, background: '#FFF8EB', borderBottom: '1px solid rgba(0,48,73,0.10)', padding: '0 16px' }}>
        <h1 className="text-[15px] font-extrabold text-[#003049] tracking-tight">{t('mobile.macro')}</h1>
      </div>
        ...
        <div className="space-y-4" style={{ padding: '14px 16px 24px' }}>
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <BigCard label={t('mobile.dep30d')} value={macro.tasaPolitica != null ? macro.tasaPolitica.toFixed(1) : null} suffix="%" delta={cmp?.tasaPolitica?.d1} />
            </div>
          </div>

          <div>
            <p className="text-[10px] uppercase tracking-widest font-semibold text-[#003049]/55 mb-1.5">{t('mobile.countryRisk')}</p>
            <BigCard label="Riesgo País" value={macro.riesgoPais} delta={cmp?.riesgoPais?.d1} />
          </div>

          <div>
            <p className="text-[10px] uppercase tracking-widest font-semibold text-[#003049]/55 mb-1.5">{t('mobile.commodities')}</p>
            <div className="grid grid-cols-3 gap-2">
              <MiniCard label={t('mobile.auGold')} value={latestCommodities.oro} />
              <MiniCard label={t('mobile.cuCopper')} value={latestCommodities.cobre} />
              <MiniCard label={t('mobile.liLithium')} value={latestCommodities.litio} />
            </div>
          </div>
